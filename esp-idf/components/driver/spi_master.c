...
static SPI_MASTER_ISR_ATTR esp_err_t check_trans_valid_nodma(spi_device_handle_t handle, spi_transaction_t *trans_desc)
{
    SPI_CHECK(handle!=NULL, "invalid dev handle", ESP_ERR_INVALID_ARG);
    spi_host_t *host = handle->host;
    SPI_CHECK( ((trans_desc->flags & (SPI_LL_IO_MODE_QIO|SPI_LL_IO_MODE_QUAD)) == 0), "Only normal io mode can be used.", ESP_ERR_INVALID_ARG );
    SPI_CHECK( (handle->cfg.spics_io_num < 0), "External CS must be used.", ESP_ERR_INVALID_ARG );
    SPI_CHECK( ((handle->cfg.cs_ena_posttrans == 0) && (handle->cfg.cs_ena_posttrans == 0)), "CS delays must be 0.", ESP_ERR_INVALID_ARG );
    SPI_CHECK( ((handle->cfg.command_bits == 0) && (handle->cfg.address_bits == 0) && (handle->cfg.dummy_bits == 0)), "cmd,addres,dummy bits must be 0.", ESP_ERR_INVALID_ARG );
    SPI_CHECK( (handle->cfg.flags & SPI_DEVICE_HALFDUPLEX) && (host->dma_chan == 0), "Only available in half-duplex mode with no DMA.", ESP_ERR_INVALID_ARG );
    // For now we can only handle 8-bit bytes transmission
    SPI_CHECK( ((trans_desc->length % 8) == 0) && ((trans_desc->rxlength % 8) == 0), "Only available in half-duplex mode with no DMA.", ESP_ERR_INVALID_ARG );
...
//---------------------------------------------------------------------------------------------------------------------------
esp_err_t SPI_MASTER_ATTR spi_device_nodma_transmit(spi_device_handle_t handle, spi_transaction_t *trans_desc, int16_t delay)
{
    esp_err_t ret =check_trans_valid_nodma(handle, trans_desc);
    if (ret != ESP_OK) return ret;
    ...
    if (trans_desc->flags & SPI_TRANS_USE_TXDATA) {
        // Send data from 'trans_desc->tx_data'
        txbuffer=(uint8_t*)&trans_desc->tx_data[0];
    } else {
        // Send data from 'trans_desc->tx_buffer'
        txbuffer=(uint8_t*)trans_desc->tx_buffer;
    }
    if (trans_desc->flags & SPI_TRANS_USE_RXDATA) {
        // Receive data to 'trans_desc->rx_data'
        rxbuffer=(uint8_t*)&trans_desc->rx_data[0];
    } else {
        // Receive data to 'trans_desc->rx_buffer'
        rxbuffer=(uint8_t*)trans_desc->rx_buffer;
    }

    // Check and adjust transfer lengths
    if ((rxbuffer != NULL) && (trans_desc->rxlength == 0)) trans_desc->rxlength = trans_desc->length;

    if (txbuffer == NULL) trans_desc->length = 0;
    if (rxbuffer == NULL) trans_desc->rxlength = 0;
    SPI_CHECK( (trans_desc->length > 0) || (trans_desc->rxlength > 0), "Nothing to transfer.", ESP_ERR_INVALID_ARG );
    ...
    SPI_CHECK( atomic_load(&host->device[handle->id]), "NO DEVICE.", ESP_ERR_INVALID_ARG );

    // Configure device
    // Reconfigure according to device settings, the function only has effect when the dev_id is changed.
    spi_setup_device(host, handle->id);
    ...
    assert(spi_ll_get_running_cmd(hal->hw) == 0);

    //spi_ll_master_set_io_mode(hal->hw, hal->io_mode);
    hal->hw->ctrl.val &= ~(SPI_FREAD_DUAL | SPI_FREAD_QUAD | SPI_FREAD_DIO | SPI_FREAD_QIO);
    hal->hw->user.val &= ~(SPI_FWRITE_DUAL | SPI_FWRITE_QUAD | SPI_FWRITE_DIO | SPI_FWRITE_QIO);

    int extra_dummy = 0;
    //when no_dummy is not set and in half-duplex mode, sets the dummy bit if RX phase exist
    if (rxbuffer && !hal->no_compensate && hal->half_duplex) {
        extra_dummy = hal->timing_conf->timing_dummy;
    }

    //SPI interface needs to be configured for a delay in some cases.
    //configure dummy bits
    spi_ll_set_dummy(hal->hw, extra_dummy + hal->dummy_bits);
    ...
    spi_ll_set_miso_delay(hal->hw, miso_delay_mode, miso_delay_num);


    // --- Wait for SPI bus ready ---
    while (hal->hw->cmd.usr);

    // --- Call pre-transmission callback, if any ---
    if (handle->cfg.pre_cb) handle->cfg.pre_cb(trans_desc);

    int remaining_bits;
    uint32_t to_transfer;
    uint32_t count, idx;
    uint32_t cs = -handle->cfg.spics_io_num;
    uint32_t *buffer32 = NULL;
    /*
    uint8_t buff_overflow;
    uint8_t buff_overflow_backup[4] = {0};
    */

    hal->hw->user.usr_mosi_highpart = 0; // use the whole spi buffer

    spi_ll_set_addr_bitlen(hal->hw, 0);
    spi_ll_set_command_bitlen(hal->hw, 0);

    spi_ll_set_command(hal->hw, 0, 0, hal->tx_lsbfirst);
    spi_ll_set_address(hal->hw, 0, 0, hal->tx_lsbfirst);

    // ==== Start the transfer ====

    gpio_set_level(cs, 0); // Activate CS line

    // Check if we have to transmit some data (MOSI phase)
    if (trans_desc->length > 0) {
        // === MOSI phase, master sends to slave ===
        hal->hw->user.usr_mosi = 1;              // mosi phase is used
        hal->hw->miso_dlen.usr_miso_dbitlen = 0; // In half duplex mode nothing will be received
        hal->hw->user.usr_miso = 0;              // miso phase not used
        remaining_bits = trans_desc->length;     // remaining bits to send
        buffer32 = (uint32_t *)txbuffer;

        // Transmit requested number of bits in up to 512 bit blocks
        while (remaining_bits > 0) {
            // Fill the spi buffer from source buffer
            // This causes ~2.66 us pause before each 64 byte block !
            to_transfer = (remaining_bits > 512) ? 512 : remaining_bits;
            count = (to_transfer + 31) / 32;
            for (idx = 0; idx < count; idx++) {
                hal->hw->data_buf[idx] = *buffer32++;
            }
            hal->hw->mosi_dlen.usr_mosi_dbitlen = to_transfer-1;
            // Start the transaction
            hal->hw->cmd.usr=1;
            // Wait until transaction finished
            while (hal->hw->cmd.usr);

            remaining_bits -= to_transfer;
        }
    }

    if (trans_desc->rxlength > 0) {
        // === MISO phase, master receives from slave ===
        if ((trans_desc->length > 0) && (delay != 0)) {
            // insert delay between mosi and miso phase
            if (delay < 0) gpio_set_level(cs, 1);
            ets_delay_us(abs(delay));
            if (delay < 0) gpio_set_level(cs, 0);
        }
        hal->hw->user.usr_miso = 1;              // miso phase is used
        hal->hw->mosi_dlen.usr_mosi_dbitlen = 0; // In half duplex mode nothing will be sent
        hal->hw->user.usr_mosi = 0;              // mosi phase not used
        remaining_bits = trans_desc->rxlength;   // remaining bits to receive
        buffer32 = (uint32_t *)rxbuffer;

        /*
        // Backup bytes from destination buffer which are going to be overwritten
        buff_overflow = (4 - ((trans_desc->rxlength % 32) / 8 )); // number of overwritten bytes (0~3)
        if (buff_overflow == 4) buff_overflow = 0;
        if (buff_overflow) {
            count = (trans_desc->rxlength / 8) - 1; // last byte index
            for (idx=0; idx<buff_overflow; idx++) {
                buff_overflow_backup[idx] = rxbuffer[count--];
            };
        }
        */
        ...
        while (remaining_bits > 0) {
            to_transfer = (remaining_bits > 512) ? 512 : remaining_bits;
            count = (to_transfer + 31) / 32;
            hal->hw->miso_dlen.usr_miso_dbitlen = to_transfer-1;
            ...
            // Copy received data to destination buffer
            // This causes ~2.66 us pause after each 64 byte block !
            for (idx = 0; idx < count; idx++) {
                *buffer32++ = hal->hw->data_buf[idx];
            }
        ...
        /*
        if (buff_overflow) {
            // Restore overwritten bytes in destination buffer
            count = (trans_desc->rxlength / 8) - 1; // last byte index
            for (idx=0; idx<buff_overflow; idx++) {
                rxbuffer[count--] = buff_overflow_backup[idx];
            };
        }
        */
    }
...
//----------------------------------------------------------
bool spi_device_uses_native_pins(spi_device_handle_t handle)
{
    return ((handle->host->flags & SPICOMMON_BUSFLAG_IOMUX_PINS) != 0);
}

//--------------------------------------------------
int spi_device_get_speed(spi_device_handle_t handle)
{
    int duty_cycle = (handle->cfg.duty_cycle_pos == 0) ? 128 : handle->cfg.duty_cycle_pos;
    return spi_get_actual_clock(APB_CLK_FREQ, handle->cfg.clock_speed_hz, duty_cycle);
}

//-------------------------------------------------------------------------
esp_err_t spi_device_set_speed(spi_device_handle_t handle, int clock_speed)
{
    spi_host_t *host = handle->host;
    int duty_cycle = (handle->cfg.duty_cycle_pos == 0) ? 128 : handle->cfg.duty_cycle_pos;
    ...
    int freq;
    spi_hal_context_t *hal = (&host->hal);
    hal->half_duplex = handle->cfg.flags & SPI_DEVICE_HALFDUPLEX ? 1 : 0;
    hal->no_compensate = handle->cfg.flags & SPI_DEVICE_NO_DUMMY ? 1 : 0;

    spi_hal_timing_conf_t temp_timing_conf;
    memcpy(&temp_timing_conf, &handle->timing_conf, sizeof(spi_hal_timing_conf_t));

    esp_err_t ret = spi_hal_get_clock_conf(hal, clock_speed, duty_cycle,
                                        !(host->flags & SPICOMMON_BUSFLAG_IOMUX_PINS),
                                        handle->cfg.input_delay_ns, &freq,
                                        &temp_timing_conf);

    SPI_CHECK(ret==ESP_OK, "assigned clock speed not supported", ret);

    handle->cfg.clock_speed_hz = clock_speed;
    handle->timing_conf = temp_timing_conf;
    // Force spi configuration
    host->prev_cs = NO_CS;

    ESP_LOGD(SPI_TAG, "SPI clock changed to %d Hz, effective clock: %d Hz", clock_speed, freq);
