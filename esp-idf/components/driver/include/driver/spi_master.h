...
/**
 * @brief Send a non-dma transaction, wait for it to complete, and return the result
 *
 * This function handles transmission of any length of data using a direct spi mode, without DMA
 *
 * @note This function is not thread safe when multiple tasks access the same SPI device.
 *      Normally a device cannot start (queue) polling and interrupt
 *      transactions simutanuously.
 ...
 * @param handle Device handle obtained using spi_host_add_dev
 * @param trans_desc Description of transaction to execute
 * @param delay Delay in us between mosi and miso phase (if both are used
 *              if negative value is given, CS is deactivated before delay and activated after delay
 * @return
 *         - ESP_ERR_INVALID_ARG   if parameter is invalid
 *         - ESP_OK                on success
 */
esp_err_t spi_device_nodma_transmit(spi_device_handle_t handle, spi_transaction_t *trans_desc, int16_t delay);
